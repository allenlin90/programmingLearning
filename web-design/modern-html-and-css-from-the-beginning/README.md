...
1. The overall problems and differences are mainly from how to set up `margin` and `padding` of the box model in a proper layout.
   <img src="./form-styling/difference-style-form.png">
...
<img src='./hotel-website/hotel-website-index.png'>
...
<img src='./hotel-website/hotel-website-about.png'>
...
<img src='./hotel-website/hotel-website-contact.png'>
...
1. We can use `::before` selector to insert content before an element. In this case, we can make a filter-like overlay to reduce the hue of the background image and make it darker without using photoshop. Note that the parent element must have `position: relative`.
   ...
   1. With filter
      <img style="width=20%" src="./edgeledger-website/with-overlay-filer.png">
   1. Without filter
      <img style="width=20%" src="./edgeledger-website/without-overlay-filer.png">
...
<img src="./hamburger-menu-overlay/hamgurger-menu-animation.gif">
...
1. Part 2 - Boxes and Arrows
   ...
   <img src="./knowledge-timeline/boxes-and-arrows.png">
...
1. Part 3 - Responsive media queries
   ...
   <img src="./knowledge-timeline/boxes-and-arrows-width-under600px.png">
...
1. Part 4 - Scroll in Animation
   ...
   <img src="./knowledge-timeline/knowledge-timeline.gif">
...
1. The followings are the CSS styling that we've learnt in the previous modules.
   ...
   <img src="./dropdown-menu/dropdownmenu-large.gif">
   ...
   * {
   ...
   <img src="./dropdown-menu/dropdownmenu-narrow.gif">
...
1. Besides, since `grid` is two-dimensional, we can use both `grid-template-columns` and `grid-template-rows` for the wrapper.
   ...
   <img src="./grid/cssgrid-rows-columns.png">
...
1. On the other hand, we can use the shorthand as other CSS properties to make the code shorter.
   ...
   ```
   <img src="./grid/cssgrid-span1.png">
...
1. We set up the layout as "**rows**" in different lines with the "tags". However, this only works if we set up the "tags" in their own classes with property `grid-area` as a variable.
     ...
     <div class="container">
       ...
       <section class="content">
         ...
         <p>
           ...
           nesciunt voluptas atque porro repellendus quia? Praesentium veritatis
           quas corporis, in enim, tenetur dolorem voluptatum labore
           exercitationem ut perferendis asperiores?
         </p>
       </section>
       <aside class="sidebar">
         <h3>Contact Us</h3>
       ...
       <div class="box-1">
         ...
         <p>
           ...
           placeat necessitatibus unde veritatis suscipit. Libero voluptatem
           earum mollitia dignissimos cumque.
         </p>
       </div>
       <div class="box-2">
         <h3>Heading</h3>
   ...
   <img src="./grid/grid-area-template.png">
...
1. We can design a "**current**" class on the buttons in the navbar, so users can have a visualized hint that which are they browsing. For example, this website has mainly 2 pages "**home**" and "**about**". We can hightlight the button in red to let users know where they are.
   <img src="./news-grid-web/navbar.png">
...
1. We create a `<div>` with "**container**" class and wrap all the contents of each section on the webpage. Therefore, contents of each section will have the same `width` and `padding` and will be centered as its `margin: auto`.
   ...
   ```
   <img src="./news-grid-web/showcase-with-overlay-and-button-style.png">
...
1. By the structure of HTML, we use asterisk `*` selector to choose the "**articles**" with spanned width, so only the first and last item can cross 2 columns.
   ...
   <img src="./news-grid-web/articles-grid.png">
...
1. We create the footer section on the homepage, which uses "Grid" and has 4 columns on the 4 rows while the last row has a single item span accross the whole row.
   <img src="./news-grid-web/footer.png">
...
1. In this case, we created a grid that has "**main content**" and a "**side column**". In the "**Grid**", we can use asterisk selector to select the 1st child `*:first-child` and give `grid-row: 1 / span 3`, so the first item on the left will extend accross the page and has triple the height of the element aside. Therefore, the number of span gives us the layout of how many items we can align on the side column. For example, if we change span to 2, we can put 2 items aside in the layout as a pattern.
   ...
   <img src="./news-grid-web/about-page.png">
...
1. Note that in this case, there are only 2 main elements in `<div class="meta">`, as we use `<small>` tag to wrap the icon and text together. Note that in the lecture, the instructor used `#article .category .category-ent` to select the `<div>` tag which is created for category tags on the page. After inspecting, this element has little margin at bottom, and we add `margin-top: 0.4rem`, so the element in centered in the container. However, we can work on that by taking off the margin aroudn the element. This will effect to the overall size of the `meta` container, as its content shrinks down (margin is removed).
   ...
   ```
   <img src="./news-grid-web/article-page.png">
...
# Protfolio website with Sass

1. The tempalte is sourced from [Mobirise](https://mobirise.com/).
1. The typeing effect by JavaScript can be checked at [Pure JavaScript Type Writer Effect
   ](https://youtu.be/POX3dT-pB4E).
1. This portfolio page has 4 sections
   1. Home
   1. About
   1. Work
   1. Contact
1. The webpage is responsive to multiple device size

### Project Setup

1. Create a `dist` directory with files and directory
   1. `index.html`
   1. img directory
   1. js directory
1. Use `npm init` to setup and `npm install node-sass -D` for Sass package. Note that `-D` flag means the package is only used for development.
1. In `package.json`, we update the `script` property as the following, so we can later run `npm run sass` to export Sass files to CSS file.
   ```json
   "scripts": {
       "sass": "node-sass -w scss/ -o dist/css/ --recursive"
   }
   ```
1. We then test the file to see if it works
   1. Create `sass` directory in root folder.
   1. Create `main.scss` and have some testing code such as
   ```scss
   $color: red;
   body {
     background: $color;
   ...
   ```
   1. Run `npm run sass` in terminal and save `main.scss` file. It should create a new `main.css` in css directory.
   1. Link the created `main.css` to HTML file and check if it's working.
1. In this project, we use `Dosis` font from Google Fonts.
1. We can separate the `scss` files into different files for main code and variables and use `@import 'fileName'` to import and use the variables in the other file. In this case, we store the variables separately in `_config.scss` file.
   ```scss
   /* _config.scss */
   // Variables
...
1.  In `#header` selecotr, I added the overlay filter learnt from the "edgeledger project (I changed the background image as well). However, the overlay filter kept covering the elements in `.header-content`. The solution is to ensure all the elements has a `position` property by referring the solution on [fix z-index](https://www.freecodecamp.org/news/4-reasons-your-z-index-isnt-working-and-how-to-fix-it-coder-coder-6bc05f103e6c/). The items in the same stack have their own priority of the stack. Usually the later the high, so the elements in the below should stack upon the elements in the upper code. However, if any element has `position` as property, the priority changes and become higher than others in the same stack. Therefore, we can just add `position: relative` to all items in `#header-home` tag with `#header-home *` selector and give not only high `z-index` but `position` property.
    ...
    <img src="./portfolio-website/header.png">
