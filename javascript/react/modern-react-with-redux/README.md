...
- [6. Understanding Lifecycle Methods](#6-understanding-lifecycle-methods)
  ...
  - [6.6. Determing Season](#66-determing-season)
...
- [11. Let's Test Your React Mastery!](#11-lets-test-your-react-mastery)
  ...
  - [11.18. Displaying a Video Player](#1118-displaying-a-video-player)
...
- [13. Navigation From Scratch](#13-navigation-from-scratch)
  ...
  - [13.6. Building a Link](#136-building-a-link)
...
- [19. Redux Store Design](#19-redux-store-design)
  ...
  - [19.4. Pure Reducers](#194-pure-reducers)
...
- [23. Handling Forms with Redux Form](#23-handling-forms-with-redux-form)
  ...
  - [23.5. Automatically Handling Events](#235-automatically-handling-events)
...
1. We imported several objects from the other JavaScript file. We can check the [demo](https://codesandbox.io/s/react-jg4cq)
    ...
    ```
    <img src="./images/first-react-app-demo.gif">
...
1. How did some content get displayed on the screen?
    ...
    1. Therefore, we get `index.html` and `bundle.js` from 2 requests. 
        <img src="./images/react-app-files.png">    
...
## 1.3. Generating a React Project
...
## 1.8. Displaying Content with Functional Components
...
# 4. Strucuturing Apps with Class-based Components
...
1. The reason that it works because `Babel` compile the JavaScript into another form which is compatible with ES5 and older browsers.
    ...
    ```
    <img src="./images/build-without-constructor-in-react.png">
...
1. We then add some inline styling for the component for the magin on the top to give some space for the component.
    ...
    ```
    <img src="./images/pics-search-bar.png">
...
## 7.13. Communicating Child to Parent
...
## 9.5. Implementing Keys in Lists
...
1. We need to assign class to the `<div>` tag which wraps all the images. Besides, we need to import the CSS file to use the style.
    ...
    export default ImageList;
    ```
    <img src="./images/pics-first-grid.png">
...
## 10.3. Creating an Image Card Component
...
1. Note that we also need to adjust CSS to render the gap between the cells in the grid with `grid-gap`. Besides, we can give 2 values to the property, as the 1st one is for horizontal, and the 2nd one is for vertical. In this case, we only need gap between cells in the vertical way.
    ...
    ```
    <img src="./images/pics-grid-final.png">
...
## 11.6. Accessing the YouTube API
...
## 11.14. Communicating from Child to Parent
1. In this react app, we have 2 main property in `state` system which are the "**video list**" returned from the API called, and the "**selected video**" which the user picks from the rendered list. 
    <img src="./images/video-app-structure.png">
...
## 11.18. Displaying a Video Player
...
1. After fixing the warnings and issues prompted from developer console, we can adjust the layout to have `VideoDetail` and `VideoList` stay side by side on the same position on the page. In this case, we can use "[grid system](https://semantic-ui.com/collections/grid.html)" from Semantic UI.
    ...
    ```
    <img src="./images/semantic-ui-grid.png">

## 11.20. Default Video Selection
...
## 12.3. Communicating the Items Prop
...
## 12.4. Building and Styling the Accordion
...
## 12.5. Helper Functinos in Function Components
...
## 12.6. Introducing useState
## 12.7. Understanding useState
...
1. In the helper function `onTitleClick`, we call `setActiveIndex` with the lastest value send with `onClick` event handler. Note that `setActiveIndex` is a setter function. When it is called, React will render the component. 
    ...
    export default Accordion;
    ```
    <img src="./images/compare-flow-class-and-functional-component.png">
1. In this case, the downside of functional component is obvious that functional component can't set or change multiple `state` properties at the same time as class-based components do.
    <img src="./images/flow-class-and-functional-component-multiple-state.png">

## 12.8. Setter Functions
...
## 12.10. Creating Additional Widgets
...
## 12.11. The Search Widget Architecture
...
1. The 2nd argument for `useEffect` for the 3 scenarios are 
    ...
    <img src="./images/use-effect2nd-arg.png">
...
## 12.18. Executing the Request from useEffect
...
## 12.23. Linking to a Wikipedia Page
...
1. In this case, we can set a function to check and wait for every `500ms` to fire the search request if the user does typing and stop. Note that this time gap for waiting can be set arbitrarily.
    <img src="./images/set-timer-flow-throttle.png">

## 12.26. Reminder on setTimeout
...
## 12.27. useEffect's Cleanup Function
...
1. This returned function will be triggered when the `state` is updated again. Then the `useEffect` will be executed once again. The flow is as 
    ...
    <img src="./images/use-effect-return-function.png">

## 12.28. Implementing a Delayed Request
...
## 12.29. Searching on Initial Render
...
## 12.30. Edge Case When Clearing Out Input Form
## 12.31. Optional Video - Fixing a Warning
...
1. In this app, we have an `if` statement to check and fire the initial search when `term` has value and `results.length` is `0`. However, after we get the response from the request call, `results` is updated, so the `useEffect` is triggered and run the code with `setTimeout` once which makes another request to the endpoint. 
    <img src="./images/component-rendering-flow-with-use-effect.png">
...
1. As we have initial value for `term` state from the beginning, `term` is assigned to `debouncedTerm`, so `useEffect` for `debouncedTerm` can fire immediately and send a request to fetch data when the app initiates. After that, the app works as the flow shown below that `debouncedTerm` will only be updated if the user change the vaule in input search bar and stop typing for more than `1000ms`, which is the given time gap in this case.
    <img src="./images/use-debounce-term169.png">
    <img src="./images/use-debounce-term169-2.png">
    ```js
    // Search.js
    const Search = () => {
        const [term, setTerm] = useState('programming');
...
## 12.33. Scaffolding the Dropdown
...
1. Besides, the dropdown list is opened by default.
    ...
    ```
    <img src="./images/dropdown-jsx173.png">

## 12.35. Selection State
...
## 12.36. Filtering the Option List
...
## 12.37. Hiding and Showing the Option List
...
## 12.39. Reminder on Event Bubbling
...
1. This is because the event objects floats up and trigger the `onClick` event handler of the dropdown list. As the handler is triggered, the dropdown list is toggled. 
    <img src="./images/event-bubbling177.png">

## 12.40. Applying What We've Learned
...
## 12.41. Binding an Event Handler
...
1. When click on the dropdown list directly, we can check from developer console that the order of the execution is 
    ...
    ```
    <img src="./images/order-event-handler-execution180.png">

## 12.43. Which Element Was Clicked?
...
## 12.45. Body Event Listener Cleanup
...
1. For this widget, we can use the same exact `Dropdown` component that we made for color changing widget.
    <img src="./images/translate-widget-structure185.png">

## 12.47. Scaffolding the Translate Component
...
1. Note that we still have some issues here that the label for `Dropdown` component was hard-coded for color changing widget, so it still shows "select a color" by default. 
    ...
    ```
    <img src="./images/scaffolding-translate-component186.png">

## 12.48. Adding the Language Input
...
## 12.49. Understanding the Convert Component
## 12.50. Google Translate API Key
...
## 12.51. Building the Convert Component
...
## 12.52. Using the Google Translate API
...
## 12.53. Displaying Translated Text
...
## 12.54. Debouncing Translation Updates
...
## 13.3. Building a Reusable Route Component
...
## 13.4. Implementing a Header for Navigation
...
1. Though this solution works at the moment, there's a downside for this and we will discuss in the later section.
    ...
    ```
    <img src="./images/react-navigation199.gif">

## 13.5. Handling Navigation
...
## 14.2. Refactoring the SearchBar
...
## 14.6. Process for Building Custom Hooks
...
## 14.7. Extracting Video Logic
## 14.8. Using the Custom Hook
...
1. After finishing development, we can bundle the file and generate 4 main static files and deploy it a web server.
    ...
    1. `image.jpg`
    1. `index.css`
    <img src="./images/react-deployment-flow216.png">
...
1. The insurance company works in 3 main functions after they receive forms from their customers
    ...
    <img src="./images/redux-analogy-insurance-company220.png">

## 16.3. A Bit More Analogy
...
## 16.5. Mapping the Analogy to Redux
...
1. The central data repository of the insurance company is the `state` that Redux manages.
    <img src="./images/redux-cycle-as-insurance-company223.png">

## 16.6. Modeling with Redux
...
1. Besides, we can use `store.getState()` to get the current `states` stored in the `store` object.
    ...
    console.log(store.getState());
    ``` 
    <img src="./images/redux-methods227.png">

## 16.10. Important Redux Notes
...
1. `Reducers` will proceed on the data and update the `state`.
    <img src="./images/redux-cycle228.png">
...
## 17.1. React Cooperating with Redux
...
## 17.2. React, Redux, and React-Redux
...
1. For the structure, if we'd like to use without `Redux`, we may have `state` as the following structure.
    <img src="./images/react-structure-without-redux232.png">
1. On the other hand, we can use `Redux` and simplify the structure.
    <img src="./images/react-structure-with-redux232.png">

## 17.4. How React-Redux Works
1. We will use `React-Redux` to create 2 new components to work on both `React` and `Redux`.
    1. `Provider`
    1. `Connect`
    <img src="./images/how-react-redux-work233.png">
...
## 17.10. Configuring Connect with MapStateToProps
...
## 17.11. Building a List with Redux Data 
...
## 17.12. Extracting more data from Redux
...
## 17.13. Calling Action Creators from Components
...
## 17.15. Functional Components with Connect 
...
1. Besides, we don't need to use any action as there's no option for users to work in this component, so we don't use any action creators in this case. 
    <img src="./images/redux-song-detail-component246.png">
...
1. We will use `/posts` for the posts and their contents and `/users/` for the authors in this project.
    <img src="./images/blog-app-structure249.png">

## 18.2. Initial App Setup
...
## 18.3. Tricking Redux with Dummy Reducers
...
## 18.5. How to Fetch Data in a Redux App
...
## 18.6. Wiring Up an Action Creator
...
## 18.7. Making a Request From an Action Creator
...
1. This is the reason why we need to use `redux-thunk` in the project to make async request for action creator.
    ...
    ```
    <img src="./images/async-action-creator-error256.png">

## 18.8. Understanding Async Action Creators
...
## 18.9. More on Async Action Creators
...
## 18.11. Behind the Scenes of Redux Thunk
...
1. Rules with Redux Thunk
    ...
    <img src="./images/how-redux-thunk-work-internnally259.png">
...
## 18.12. Shortened Syntax with Redux Thunk
...
## 19.2. Return Values from Reducers
1. Rules of Reducers
...
1. When the app initiates, it will check if there's `undefined` from each reducer to know if the app is in the initial stage. 
    <img src="./images/reducer-being-called-first-time263.png">
...
1. Therefore, the returned value from the reducer should be purely from the previous `state` and the action. 
    <img src="./images/pure-reducer264.png">

## 19.5. Mutations in JavaScript
1. Rules of Reducers
    1. Must return **_ANY_** value besides `undefined`.
    1. Produces `state`, or data to be used inside of the app using only previous state and the action (reducers are pure).
    1. Must not return reach `out of itself` to decide what value to return.
...
## 19.6. Equality of Arrays and Objects
...
## 19.8. Safe State Updates in Reducers
1. There are several ways to manipulate arrays and objects in JavaScript. The main point is to avoid modifying the `state` object directly.
    <img src="./images/safe-state-updates-reducers268.png">
...
## 19.9. Switch Statements in Reducers
...
## 19.10. Dispatching Correct Values
...
1. The other way to work around
    ...
    <img src="./images/flow-fetching-user273.png">

## 19.13. Fetching Singular Records
...
1. Note that the data structure respond by json placeholder is different that if we fetch only a single user, the endpoint will return an object. However, if we don't give the specific user to request, the endponit will return an array of objects. 
    ...
    ```
    <img src="./images/fetch-singular-records274.png">

## 19.14. Displaying the User Header
...
## 19.16. Extracting Logic to MapStateToProps
...
1. However, by checking in "**Network**" > "**XHR**" in developer console, we can notice that we have requested to the endpoint for the same users multiple times. 
    <img src="./images/fetching-same-user-multi-times278.png">

## 19.18. Memoizing Functions
...
## 19.21. Alternate Overfecthing Solution
...
## 19.22. Action Creators in Action Creators
...
## 19.24. Quick Refactor with Chain
...
1. Besides, multiple streamers (video feeders) can connect and feed videos to the RTMP server at the same time.
    <img src="./images/twitch-clone-app-structure287.png">
1. Besides the RTMP server, we need another server to provide the list of streaming videos that are available or connected to RTMP server. Note that RTMP server only serves a purpose to allow connection between streamers and viewers.
    <img src="./images/twitch-clone-app-outline287.png">

## 20.2. Mockups in Details
...
1. When using `BrowserRouter` object, it will internally create `history` object. This `history` object will track on the path that the user visits.
    <img src="./images/react-router-structure292.png">

## 20.7. How Paths Get Matched
...
1. The keyword changes the behavior of how does React Router evaluates the path extracted from the search bar. 
    ...
    ```
    <img src="./images/how-react-route-works293">

## 20.8. How to "Not" Navigate with React Router
...
## 20.9. Navigating with React Router
...
## 20.13. Always Visible Components
...
1. We can put the component out of `BrowserRouter`, so it will be visible from all the routes with the other components.
    <img src="./images/structure-stream-app300.png">    

## 20.14. Connecting the Header
...
1. However, the current set up is not finished yet, and we will get an error message indicating that we are using `Link` out of a `Router`.
    ...
    ```
    <img src="./images/error-to-use-link-out-of-router301.png">

## 20.15. Links Inside Routers
1. To solve the issues from the last section, we can change the structure and move`Header` component in to `BrwoserRouter` to be right above other components. Besides, since `Header` is not controlled with `Route`, it will show in every route. 
    ...
    ```
    <img src="./images/links-inside-route302.png">



# 21. Handling Authenitcation with React
## 21.1. OAuth-Based Authentication
...
## 21.2. OAuth for Servers vs Browser Apps
...
1. Note that the callback should also listen to the user action that if the user has logged out from Google.
    <img src="./images/google-browser-oauth306.png">

## 21.3. Creating OAuth Credentials
...
## 21.4. Wiring Up the Google API Library
...
## 21.5. Sending a User Into the OAuth Flow
...
## 21.6. Rendering Authentication Status
...
## 21.8. Displaying Sign in and Sign Out
...
## 21.9. On-Demand Sign in and Sign Out
...
## 21.10. Redux Architecture Design
...
## 21.12. Connecting Auth with Action Creators
...
## 21.13. Building the Auth Reducer
...
## 21.14. Handling Auth Status Through Redux
...
# 22. Reducx Dev Tools
## 22.1. Using Redux Dev Tools to Inspect the Store
...
1. We can not only track the state and actions along the useflow but also click "jump" on each action to check the state at each action. 
    <img src="./images/redux-dev-tool-extension321.png">
...
## 22.2. Debug Sessions with Redux Dev Tools
...
1. In regular conditions, we create a component scope `state` which listen and handles all the changes to the input data to the component. 
    <img src="./images/handling-inputs-without-redux324.png">
1. By using `redux-form`, we can skip the repetitive process to keep wiring up Redux `store` with `maptStateToProps` and `Action Creator` betwen Redux and the component.
    <img src="./images/handling-inputs-with-dom324.png">

## 23.2. Useful Redux Form Examples
...
1. By using `Field` we have the first part connected by Redux Form library automatically.
    <img src="./images/create-forms327.png">

## 23.5. Automatically Handling Events
...
1. We can give the render helper function an argument which will be passed by `Field` component.
    <img src="./images/automatically-handle-event-by-field328.png">
...
1. In general condition, we'd like to have a button for users to click and submit the request to create a stream in the App. In the callback function passing to the `onSubmit` event handler, we will have `event` argument and call its `event.preventDefault()` method to prevent redicting the user.
    ...
    ```
    <img src="./images/handle-form-submit330.png">
...
1. However, we should validate the input from users and prevent having empty string or invalid data when the submit event fires. 
    ...
    ```
    <img src="./images/print-form-args330.png">
    
## 23.8. Validation of Form Inputs
...
## 23.9. Displaying Validation Messages
...
1. Therefore, we will see the error message directly when we firstly visit the page, as there's nothing in the input, so the validators run and return error message 
    ...
    ```
    <img src="./images/displaying-validation-messages332.png">

## 23.10. Showing Errors on Touch
...
## 23.11. Highlighting Errored Fields
1. To solve the issue from the last section, we can simply add `error` to the class of `form` element.
    ...
    ```
    <img src="./images/highlight-error-message-fields334.gif">
...
1. The list of available streams will be then sent back to the React app for users to select which stream that the user wants to watch.
    <img src="./images/creating-stream335.png">

## 24.2. REST-ful Conventions
1. We will use JSON Server with REST-ful conventions. This is a conventional design pattern which makes developers working easier with each others.
    <img src="./images/restful-convention336.png">

## 24.3. Setting Up an API Server
...
## 24.4. Creating Streams Through Action Creators
...
## 24.5. Creating a Stream with REST Conventions
...
## 24.6. Dispatching Actions After Stream Creation
...
1. The REST API we set up will respond data accordingly. According to the chart, as we have known what are the response from each action. Therefore, if we follow the REST API in convention, we can create the action creators for all the operations. 
    <img src="./images/response-from-rest-api341.png">
...
1. The reason to use an object rather than an array is that it's easier and more efficient to edit items in the collection.
    <img src="./images/object-based-reducer342.png">
...
## 24.10. Handling Fetching, Creating, and Updating
...
## 24.11. Deleting Properties with Omit
...
## 24.12. Merging Lists of Records
1. As we will receive an array of objects from the endpoint, can use `_.mapKeys(array, key)` to turn the array into an object which use the given key of each object in the array as the key.
    <img src="./images/map-keyslodash346.png">
    ```js
    // sammple code
    const colors = [
        {hue: 'green'},
...
## 24.13. Fetching a List of All Streams
...
1. After wiring up, we can check `state` from `Redux DevTools`. Note that we should have the JSON DB running on port 3031. In this case, we have had 2 instances stored in the database when we work on `StreamCreate` component.
    ...
    ```
    <img src="./images/fetch-all-streams347.png">

## 24.14. Rendering All Streams
...
## 24.15. Associating Streams with Users
...
## 24.16. Conditionally Showing Edit and Delete
...
## 24.17. Linking to Stream Creation
...
1. The `BrowserRouter` we use to navigate between routes creates a `history` object which keeps track of the address bar in the browser.
    <img src="./images/history-reference353.png">
...
1. So if we want to call it in the action creator, we have to pass it from the component to the action creator.
    <img src="./images/browser-router-with-history-object353.png">
1. The other way to work around is to create `history` object ourselves.

## 24.20. Creating a Browser History Object
...
## 24.21. Implementing Programmatic Navigation
...
## 24.22. Manually Changing API Records
...
## 24.26. Selecting Records from State
...
1. This is because if we access `/streams/edit/:id` directly, `streams` is not updated in Redux store, so there's no `stream` data can be rendered from `state.streams`.
    ...
    ```
    <img src="./images/undefined-stream-from-redux-store361.png">

## 24.27. Component Isolation with React Router
...
## 24.28. Fetching a Stream for Edit Stream
...
## 24.30. Refactoring Stream Creation
...
1. However, there's an issue for the callback function and giving `console.log`. This will be introduced in the next section. 
    ...
    ```
    <img src="./images/setting-initial-value366.png">

## 24.32. Avoiding Changes to Properties
...
1. In the app, we can see that the 4th Stream which is owned by the user has missing "Edit" and "Delete" buttons and the 4th Stream object has no `userId` property.
    <img src="./images/pu-tvs-patc-hhttp-request369.png">
...
## 25.1. Creating a Portal
1. A `Modal` is a component similar to `alert` that shows message while blocking allow the other UI on the screen. The component is also called `Dialog`.
2. If we nest the `Modal` and use in the component, its parent DOM node will be the component calling it.
3. In most of the cases, we'd like the `Modal` nested in the `body` or `div#root` tag directly to cover all the elements on the screen.
4. This is similar to `teleport` tag in Vue which can send a component to any parent tag in the project app structure.
5. The common use cases for a React Portal is when handling the `Modal` in the app structure or if the HTML is rendered by other server-side frameworks other than the React App itself. Therefore the React components should be injected in to a specific tag in HTML.
6. We create a new tag in `index.html` as a sibling to `#root`. Note that we can't inject to `#root` directly or it will replace all the contents in the tag.
    ```js
    ...
    ```
7. We can import and use `Modal` component in `StreamDelete`.
    ```js
...
## 25.2. Hiding a Modal
1. We can firstly update the UI to make it looks prettier.
    ```js
    // src/components/Modal.js
    import React from 'react';
    import ReactDOM from 'react-dom';
    ...
    export default Modal;
    ```
2. Besides, we'd like to hide the component when the user click 'Cancel' or click outside of the `Modal`.
3. From the current app structure, the `Modal` only shows when the users is on `/streams/delete` route.
4. Therefore, we could navigate the user away to close the `Modal` when the user clicks on the overlay that covers the screen or the `Cancel` button.
5. However, through the current setting, if the user clicks any child component, the click event will bubbles up and force the user navigates as the user clicks on the overlay or cancel.
6. Therefore, on the first direct child component, we can call `event.stopPropagation` to stop bubbling the click event.
7. In addition, the current `Modal` is hard-coded with its content and isn't reusable for other components and scenarios.
8. We can refactor it to receive custom contents while having default settings to make it more generic.
    ```js
    // src/components/Modal.js
    import React from 'react';
    import ReactDOM from 'react-dom';
...
## 25.3. Making the Modal Resuable
1. We can pass contents and JSX components from the component calling `Modal`.
2. However, by using a `div` tag to wrap the `buttons`, it may cause styling problems when inserting the elements.
3. In this case, we can use React fragments `<></>` as a wrapper because React, similar to Vue, requires a single node to import or export.
    ```js
    // src/components/streams/StreamDelete.js
    import React from 'react';
    import Modal from '../Modal';
...
## 25.4. React fragments
1. There are 2 syntax to use [`React.Fragment`](https://zh-hant.reactjs.org/docs/fragments.html), we can use the full name or simply give an empty tag `<></>`.
2. Note that `React.Fragment` only supports `key` attribute when we render a list, but it doesn't support other HTML attributes.
3. Besides, to use `key`, we must declare the fragment with full name. The simplified syntax `<></>` can't take any attribute.
    ```js
    // src/components/streams/StreamDelete.js
    import React from 'react';
    import Modal from '../Modal';
...
## 25.5. onDismiss from the parent
1. To make `Modal` more reusable, 
    ```js
    // src/components/streams/StreamDelete.js
    import React from 'react';
    import Modal from '../Modal';
...
# 26. Reminder on path params
1. To make the `Modal` more useful, we can pass the "stream" that we want to delete in this case. Otherwise, the user may not be aware which stream is going to be removed.
2. We can pass the stream ID in the route such as `/streams/delete/:id` in this case.
3. Therefore, we can firstly update the routing setup in `App.js`.
    ```js
    // src/components/App.js
    import React from 'react';
    import { Router, Route } from 'react-router-dom';
    ...
    export default App;
    ```
4. We then update the component update the "delete" button to `Link` and give the stream ID in `renderAdmin`.
    ```js
    // src/components/streams/StreamList.js
    import React from 'react';
    import { connect } from 'react-redux';
        ...
        if (stream.userId === this.props.currentUserId) {
          return (
            <div className='right floated content'>
              ...
              // turn to Link and give stream ID
              <Link
...
# 27. Fetching the deletion stream
1. To use the lifecyle hook, we can turn `StreamDelete` from functional to class component.
2. Besides, to use the actions with redux, we can import `connect` and wire the component up.
    ```js
    import React from 'react';
    import { connect } from 'react-redux';
    import Modal from '../Modal';
...
## 27.1. Conditionally showing stream details
```js
// src/components/streams/StreamDelete.js
import React from 'react';
import { connect } from 'react-redux';
...
2. After the stream is removed and updated to Redux store, we can redirect the user back to the root route `/`.
    ...
    renderActions() {
        ...
        <React.Fragment>
            ...
            <Link to='/' className='ui button' type='button'>
            Cancel
            </Link>
        </React.Fragment>
        );
...
1. We will use [node-media-server](https://www.npmjs.com/package/node-media-server) to set up RTMP server.
    <img src="./images/388-rtmpserver.png" />
...
1. We can set up scene in OBS with chosen display and audio input device.
    <img src="./images/390-obs-scene-setup.png" />
...
4. The setting shows a default video player in the component.
    ...
    export default connect(mapStateToProps, { fetchStream })(StreamShow);
    ```
    <img src="./images/392-flvjs-player.png" />
...
# 31. Working with Older Versions of React

# 32. Ajax Requets with React 

# 33. Modeling Application State

# 34. Managing App State with Redux

# 35. Intermediate Redux: Middleware

# 36. React Router + Redux Form v6

# 37. Bonus Topics

# 38. React Router + Redux Form v4
